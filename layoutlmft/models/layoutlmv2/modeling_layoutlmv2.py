...
class LayoutLMv2SelfAttention(nn.Module):
    ...
    def forward(
        ...
        # calculate attention scores based on gumbel sampling
        attention_probs = F.softmax(attention_scores, dim=-1, dtype=torch.float32).type_as(value_layer)
        attention_probs = gumbel_attention_mask * attention_probs
...
class LayoutLMv2Attention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.self(
            ...
            gumbel_attention_mask=gumbel_attention_mask,
        )
...
class LayoutLMv2Encoder(nn.Module):
    ...
    def _cal_gumbel_attention_mask(self, bbox, train_or_val):
        """ get the attention mask after gumbel sampling. """
        def inverse_sigmoid(x):
            return torch.log(x / (1.0 - x))

        # gumbel sigmoid function
        def gumbel_sigmoid(y_select, tau=1.0):
            m = Gumbel(torch.tensor([0.0]), torch.tensor([1.0]))
            g1 = m.sample(y_select.shape).cuda().squeeze(-1)
            g2 = m.sample(y_select.shape).cuda().squeeze(-1)
            return torch.exp((y_select + g1) / tau) / (torch.exp((y_select + g1) / tau) + torch.exp(g2 / tau))

        # get relative position of inputs
        position_coord_x = bbox[:, :, 0]
        position_coord_y = bbox[:, :, 3]
        rel_pos_x_2d_mat = (position_coord_x.unsqueeze(-2) - position_coord_x.unsqueeze(-1)) / (torch.max(position_coord_x) - torch.min(position_coord_x))
        rel_pos_y_2d_mat = (position_coord_y.unsqueeze(-2) - position_coord_y.unsqueeze(-1)) / (torch.max(position_coord_y) - torch.min(position_coord_y))

        y_select = torch.exp(- self.alpha * torch.pow(rel_pos_y_2d_mat.detach(), 2)) - 0.01
        # training
        if train_or_val == 'train':
            y_select = inverse_sigmoid(y_select)
            y_action = gumbel_sigmoid(y_select, tau=0.35)
            y_action = y_action.unsqueeze(1)

            return y_action
        # inferring
        else:
            y_action = torch.where(y_select >= 0.5, 1.0, 0.0)
            y_action = y_action.unsqueeze(1)
            return y_action
    ...
    def forward(
        ...
        gumbel_attention_mask = self._cal_gumbel_attention_mask(bbox, train_or_val)
...
class LayoutLMv2Model(LayoutLMv2PreTrainedModel):
    ...
    def _calc_other_embeddings(self, bbox, position_ids, token_type_ids):
        if token_type_ids is None:
        ...
        spatial_position_embeddings = self.embeddings._cal_spatial_position_embeddings(bbox)
        token_type_embeddings = self.embeddings.token_type_embeddings(token_type_ids)
        other_embeddings = position_embeddings + spatial_position_embeddings + token_type_embeddings
...
class Memory(nn.Module):
    ...
    def update(self, input_idx, emb):
        # update queue
        input_idx = int(input_idx)

        # create a queue for a new token
        if input_idx not in self.memory:
            queue_len = min(quelen, self.id2sum[input_idx])    # queue length
            self.memory[input_idx] = {
                'queue_len':    queue_len,
                'cnt':          0,                             # indicating the current index
                'flag':         0,                             # indicating whether the queue is full
                'emb':          torch.zeros((queue_len, 768), dtype=torch.float),
            }

        cnt = self.memory[input_idx]['cnt']
        queue_len = self.memory[input_idx]['queue_len']

        # update embedding
        self.memory[input_idx]['emb'][cnt] = emb

        self.memory[input_idx]['cnt'] = (cnt + 1) % queue_len
        if cnt + 1 == queue_len:
            self.memory[input_idx]['flag'] = 1

    def query(self, input_idx):
        # query embedding according to token
        input_idx = int(input_idx)
        ...
        cnt = self.memory[input_idx]['cnt']
        flag = self.memory[input_idx]['flag']
        emb = self.memory[input_idx]['emb']
        if flag == 1:
            return torch.flip(torch.cat([emb[cnt:], emb[:cnt]], dim=0), dims=[0])
        else:
            return torch.flip(emb[:cnt], dims=[0])
...
class LayoutLMv2ForTokenClassification(LayoutLMv2PreTrainedModel):
    def __init__(self, config):
        ...
        global alpha, quelen
        alpha = config.alpha
        quelen = config.quelen
        self.mem = Memory(config.id2sum)
        self.num_labels = config.num_labels
        self.layoutlmv2 = LayoutLMv2Model(config)
        self.position_embeddings = nn.Embedding(110, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # self.layernorm = LayerNorm(config.hidden_size, eps=1e-12)
        self.out_dropout = nn.Dropout(config.hidden_dropout_prob)
        self.pos_dropout = nn.Dropout(config.hidden_dropout_prob)

        self.classifier0 = nn.Linear(config.hidden_size, config.num_labels)
    ...
    def forward(
        ...
        attention_head_size = int(768 / 8)

        # cross-document attention
        document = torch.zeros((batch_size, seq_length, 768)).cuda()
        for i in range(input_ids.shape[0]):
            for j in range(input_ids.shape[1]):
                if token_ids[i, j] == -1:
                    break

                # query embedding of queue
                hidden = self.mem.query(input_ids[i, j])
                if hidden is None:
                    hidden = sequence_output[i, j, :].unsqueeze(0)
                else:
                    hidden = torch.cat([sequence_output[i, j, :].unsqueeze(0), hidden.cuda().detach()], dim=0)

                # position embedding
                pos_ids = torch.arange(hidden.shape[0], dtype=torch.long).cuda()
                pos_emb = self.position_embeddings(pos_ids)
                hidden = hidden + pos_emb
                hidden = self.pos_dropout(hidden)
                
                first_hidden = hidden[0]

                # attention
                q = self.linear_q(first_hidden).view(num_attention_heads, 1, attention_head_size)
                k = self.linear_k(hidden).view(num_attention_heads, -1, attention_head_size)
                v = self.linear_v(hidden).view(num_attention_heads, -1, attention_head_size)
                score = F.softmax(torch.matmul(q, k.transpose(-1, -2)) / math.sqrt(attention_head_size), dim=-1)
                # print(score[:, -1])
                out = torch.matmul(score, v).view(-1)
                out = self.dense(out)
                out = self.out_dropout(out)
                out = out + first_hidden
                document[i, j, :] = out

        # update queue while training
        if train_or_val == 'train':
            for i in range(input_ids.shape[0]):
                for j in range(input_ids.shape[1]):
                    if token_ids[i, j] == -1:
                        break
                    self.mem.update(input_ids[i, j], sequence_output[i, j, :])

        logits = self.classifier0(document)
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                # active_logits_1 = logits_1.view(-1, self.num_labels)[active_loss]

                active_labels = labels.view(-1)[active_loss]
                loss = loss_fct(active_logits, active_labels) # + 0.2 * loss_fct(active_logits_1, active_labels)
                # b = 0.1
...
# class baseline_LayoutLMv2ForTokenClassification(LayoutLMv2PreTrainedModel):
#     def __init__(self, config):
#         super().__init__(config)
#         self.num_labels = config.num_labels
#         self.layoutlmv2 = LayoutLMv2Model(config)
...
#
#         self.init_weights()
#         print('baseline token classification...')
#
#     def get_input_embeddings(self):
#         return self.layoutlmv2.embeddings.word_embeddings
#
