...
_URL = "/root/.cache/huggingface/datasets/source/"
...
class XFUN(datasets.GeneratorBasedBuilder):
    ...
    # tokenizer = AutoTokenizer.from_pretrained("/root/.cache/huggingface/xlm-roberta-base/")
            ...
            features=datasets.Features(
                {
                    ...
                    "train_or_val": datasets.Value("string"),
                    "token_ids": datasets.Sequence(datasets.Value("int64")),
                }
            ),
                    ...
                    item.update(
                        {
                            ...
                            "img_path": doc['img']['fpath'],
                            "image": image,
                            "entities": entities_in_this_span,
                            "relations": relations_in_this_span,
                            "train_or_val": train_or_val,
                            "token_ids": list(range(token_idx, token_idx+input_ids_len)) + [-1] * (maxlen - input_ids_len),
                        }
                    )
                    token_idx += input_ids_len

                    # print(input_ids_len)
                    # print(item['token_ids'])
